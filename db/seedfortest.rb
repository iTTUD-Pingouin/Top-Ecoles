...
# PARSING FIRST FILE = FR-annuaire-education ------------------------------------------------------------------------------------------
...
puts "fr-en-annuaire-education csv file parsed successfully !!!!"



filepath = 'db/lyceecollegefrance.csv'

puts "Parsing lyceecollegefrance csv file ..."
CSV.foreach(filepath, csv_options) do |row|
  if College.find_by(uai: row["code UAI"][0...8])
    college = College.find_by(uai: row["code UAI"][0...8])
    college.departement = row['département']
    college.academie = row['académie']
    college.region = row['région']
    college.latitude = row['latitude (Y)']
    college.longitude = row['longitude (X)']
    college.save!

  elsif Lycee.find_by(uai: row["code UAI"][0...8])
    lycee = Lycee.find_by(uai: row["code UAI"][0...8])
    lycee.departement = row['département']
    lycee.academie = row['académie']
    lycee.region = row['région']
    lycee.latitude = row['latitude (Y)']
    lycee.longitude = row['longitude (X)']
    lycee.save!
  end
end

puts "Parsing lyceecollegefrance over succesfully ! "
